...
interface ConfirmedLog {
  ...
  createdAt?: string;
  updatedAt?: string;
  extractedData?: DiveData; // Add extractedData for modal display
}
...
function mapApiToConfirmedLog(
  ...
  console.log("[DEBUG] mapApiToConfirmedLog processing item:", {
    id: item.id,
  ...
  console.log("[DEBUG] mapApiToConfirmedLog result:", {
    id: result.id,
    date: result.date,
...
const DiveLog: React.FC = () => {
  ...
  useEffect(() => {
    async function mapConfirmedLogs() {
      ...
      // Sort confirmed logs by session_date (newest first)
      ...
      setConfirmedLogsWithData(sorted);
    }
  ...
  const handleManualEntrySave = async (data: DiveData) => {
    try {
      ...
      if (!diverId) {
        alert("Error: Could not find diver ID for the selected diver.");
        return;
      }

      // Clean the data for manual entries - include empty Reps array for consistency
      const cleanedData = {
        Name: data.Name,
        ...
        })).filter((dive) => dive.DiveCode && dive.Board && dive.DrillType), // Filter out empty dives
        comment: data.comment || "",
        rating: data.rating,
        balks: data.balks || 0,
      };

      // Validate that we have at least one valid dive
      if (cleanedData.Dives.length === 0) {
        alert("Error: At least one valid dive is required.");
        return;
      }
      ...
      // Add session_date if present
      if (data.session_date) {
        payload.session_date = data.session_date;
      }

      console.log("[DEBUG] Manual entry payload:", payload);
      console.log("[DEBUG] Manual entry cleaned data:", cleanedData);
      console.log("[DEBUG] Session date being sent:", data.session_date);
      console.log("[DEBUG] Session date type:", typeof data.session_date);
      const response = await updateTrainingData(payload);
      console.log("[DEBUG] Manual entry response:", response);
      console.log(
        "[DEBUG] Response session_date:",
        response.data?.session_date || response.session_date
      );
      ...
      const newLog: ConfirmedLog & { extractedData?: DiveData } = {
        id: newLogId,
        diverName: data.Name,
        ...
        extractedData: cleanedData, // Include the extracted data for the modal
      };

      console.log("[DEBUG] New log date field:", newLog.date);
      console.log("[DEBUG] New log session_date field:", newLog.session_date);
      console.log("[DEBUG] Date parsing test:", {
        original: data.session_date,
        parsed: data.session_date ? new Date(data.session_date) : null,
        toLocaleDateString: data.session_date
          ? new Date(data.session_date).toLocaleDateString()
          : null,
        getTime: data.session_date
          ? new Date(data.session_date).getTime()
          : null,
      });
