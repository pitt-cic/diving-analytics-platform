...
const DiveLog: React.FC = () => {
    ...
    const handleManualEntrySave = async (data: DiveData) => {
        try {
            ...
            // Clean the data for manual entries - remove Reps array
            const cleanedData = {
                ...data,
                Dives: data.Dives.map(dive => ({
                    ...
                    // Reps array is intentionally excluded for manual entries
                }))
            };
            ...
            const newLog: ConfirmedLog = {
                id: response.id || `manual-${Date.now()}`, // Use response ID or fallback
                diverName: data.Name,
                date: new Date().toLocaleDateString(),
            ...
            alert("Failed to save manual training log. Please try again.");
        }
    };
